import Schema, { Type } from 'schema-verify';
...
class Base {
  ...
  execAll(queryList) {
    queryList = queryList || [];
    const execute = this._execute;
    queryList = Type.array.isNotEmpty(queryList) ? queryList : Store$1.getStore();
...
class Order extends Base {
  ...
  build() {
    ...
    const orderInfos = Type.array.safe(this.orderInfos);
...
class Insert extends Query {
  ...
  data(data) {
    ...
    const insertData = Type.object.safe(this.insertData);
  ...
  fields(arg, ...otherArgs) {
    ...
    const insertFields = Type.array.safe(this.insertFields);
  ...
  multiData(dataArr) {
    ...
    const insertDataArr = Type.array.safe(this.insertDataArr);
...
class Term extends Base {
  ...
  termsBuild() {
    ...
    const allTermStr = !Type.array.isNotEmpty(termBrackets) ? this.formatTerms(termInfos) : this.formatTermBrackets(termBrackets, termInfos);
    return allTermStr;
  }
  ...
  formatTermBrackets(brackets, terms) {
    ...
    for (let i = 0; i < bracketsLen; i++) {
      ...
      allTermStr = Type.string.isNotEmpty(allTermStr) ? `${allTermStr} ${termStr}` : termStr;
    }
  ...
  termCache(data, sign, logic) {
    ...
    const termInfos = Type.array.safe(this.termInfos);
    const termsArr = [];
  ...
  bracketTerm(logic) {
    const termInfos = Type.array.safe(this.termInfos);
    const termBrackets = Type.array.safe(this.termBrackets);
...
class TermApi extends Query {
  constructor() {
    ...
    this.equal = data => this;

    this.notEqual = data => this;

    this.in = data => this;

    this.notIn = data => this;

    this.more = data => this;

    this.less = data => this;

    this.moreEqual = data => this;

    this.lessEqual = data => this;

    this.like = data => this;

    this.notLike = data => this;

    this.between = data => this;

    this.notBetween = data => this;

    this.orEqual = data => this;

    this.orNotEqual = data => this;

    this.orIn = data => this;

    this.orNotIn = data => this;

    this.orMore = data => this;

    this.orLess = data => this;

    this.orMoreEqual = data => this;

    this.orLessEqual = data => this;

    this.orLike = data => this;

    this.orNotLike = data => this;

    this.orBetween = data => this;

    this.orNotBetween = data => this;
...
class Where extends TermApi {
  constructor() {
    ...
    this.whereEqual = data => this;

    this.whereNotEqual = data => this;

    this.whereIn = data => this;

    this.whereNotIn = data => this;

    this.whereMore = data => this;

    this.whereLess = data => this;

    this.whereMoreEqual = data => this;

    this.whereLessEqual = data => this;

    this.whereLike = data => this;

    this.whereNotLike = data => this;

    this.whereBetween = data => this;

    this.whereNotBetween = data => this;

    this.whereOrEqual = data => this;

    this.whereOrNotEqual = data => this;

    this.whereOrIn = data => this;

    this.whereOrNotIn = data => this;

    this.whereOrMore = data => this;

    this.whereOrLess = data => this;

    this.whereOrMoreEqual = data => this;

    this.whereOrLessEqual = data => this;

    this.whereOrLike = data => this;

    this.whereOrNotLike = data => this;

    this.whereOrBetween = data => this;

    this.whereOrNotBetween = data => this;
...
class Having extends Where {
  constructor() {
    ...
    this.havingEqual = data => this;

    this.havingNotEqual = data => this;

    this.havingIn = data => this;

    this.havingNotIn = data => this;

    this.havingMore = data => this;

    this.havingLess = data => this;

    this.havingMoreEqual = data => this;

    this.havingLessEqual = data => this;

    this.havingLike = data => this;

    this.havingNotLike = data => this;

    this.havingBetween = data => this;

    this.havingNotBetween = data => this;

    this.havingOrEqual = data => this;

    this.havingOrNotEqual = data => this;

    this.havingOrIn = data => this;

    this.havingOrNotIn = data => this;

    this.havingOrMore = data => this;

    this.havingOrLess = data => this;

    this.havingOrMoreEqual = data => this;

    this.havingOrLessEqual = data => this;

    this.havingOrLike = data => this;

    this.havingOrNotLike = data => this;

    this.havingOrBetween = data => this;

    this.havingOrNotBetween = data => this;
...
class Func extends Base {
  funcField(func, field) {
    const needSafeTrans = Type.string.isNotEmpty(field) && field !== "*";
    const fieldStr = needSafeTrans ? field : Type.number.is(field) ? field + "" : "*";
...
class Combine extends Having {
  ...
  groupBy(...fields) {
    let groupByFields = Type.array.safe(this.groupByFields);
  ...
  formatFuncs() {
    const combineFuncs = Type.array.safe(this.combineFuncs);
    let funcs = [];
  ...
  funcFeilds(...funcInfos) {
    for (let info of funcInfos) {
      ...
      if (funcInputVerify(info)) {
        ...
        if (Type.object.is(funcCase) && Type.func.is(funcCase[func])) {
...
class Join extends Combine {
  ...
  getQueryTables() {
    const queryTables = Type.array.safe(this.queryTables).map(table => {
  ...
  formatJoinFields() {
    const tableFieldsMap = Type.object.safe(this.tableFieldsMap);
    const tableFieldsAsMap = Type.object.safe(this.tableFieldsAsMap);
    const result = [];

    for (const table in tableFieldsMap) {
      const fields = Type.array.safe(tableFieldsMap[table]);
      const asMap = Type.object.safe(tableFieldsAsMap[table]);
  ...
  joinBuild(query) {
    const joinTypeInfos = Type.array.safe(this.joinTypeInfos);
    ...
    for (const typeInfo of joinTypeInfos) {
      ...
      const info = Type.object.safe(typeInfo.info);
      const tableName = info.tableName;
      const termInfos = Type.array.safe(info.termInfos);
  ...
  multiTables(arg, ...otherArgs) {
    const queryTables = Type.array.safe(this.queryTables);
    const args = argStrArrTrans(arg, otherArgs);
    const tables = [];
...
class Select extends Join {
  ...
  formatFields() {
    const fields = Type.array.safe(this.selectFields);
    const asMap = Type.object.safe(this.fieldsAsMap);
    const result = [];
  ...
  fields(arg, ...otherArgs) {
    const selectFields = Type.array.safe(this.selectFields);
    const args = argStrArrTrans(arg, otherArgs);
    const fields = [];
...
class Update extends Where {
  ...
  updateCache(data, type) {
    ...
    const updateInfos = Type.object.safe(this.updateInfos);
...
class Alter extends Base {
  ...
  alterCache(method, field, alterField) {
    if (AlterMethods.drop !== method && !Type.object.isNotEmpty(alterField)) {
    ...
    const alterInfos = Type.array.safe(this.alterInfos);
...
class BaseConnect {
  ...
  setConfig(config) {
    ...
    connectionLimit = Type.number.isNatural(connectionLimit) ? connectionLimit : 1;
    let dbConfig = {
      host,
      user,
      password,
  ...
  getDbConnect() {
    ...
    return new Promise((relsove, reject) => {
      pool.getConnection((err, connection) => {
        ...
        if (!connection || Type.func.isNot(connection.query) || Type.func.isNot(connection.release)) {
...
class Builder {
  ...
  initInstance(type, instance) {
    ...
    if (Type.string.isNotEmpty(queryTable) && Type.func.is(instance.table)) {
...
function SqlQuery(config) {
  ...
  if (Type.object.isNot(config)) {
    ...
    if (Type.string.isNotEmpty(config) && DialectTypes[config]) {
      dialect = config;
    }

...
function analyTmpl(tmpl, opts) {
  return tmpl.replace(/\{\{([a-zA-Z_0-9]+)\}\}/g, (match, key) => {
...
class Term extends Base {
  ...
  termsBuild() {
    ...
    return !Schema.Type.array.isNotEmpty(termBrackets) ? this.formatTerms(termInfos) : this.formatTermBrackets(termBrackets, termInfos);
  }
...
class Where extends TermApi {
  constructor() {
    ...
    for (const api of Object.keys(WHERE_TERM_API)) {
      const whereTermApi = WHERE_TERM_API[api];
...
class Having extends Where {
  constructor() {
    ...
    for (const api of Object.keys(HAVING_TERM_API)) {
      const havingTermApi = HAVING_TERM_API[api];
...
class Join extends Combine {
  ...
  getQueryTables() {
    return Schema.Type.array.safe(this.queryTables).map(table => {
...
class Builder {
  ...
  initInstance(type, instance) {
    ...
    if (Schema.Type.func.is(instance.setExecute) && execute) {
